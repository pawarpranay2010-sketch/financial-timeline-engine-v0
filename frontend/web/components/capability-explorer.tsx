    ...
    <Card className="border-border/70">
      <CardContent className="pt-5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Badge variant="outline" className={cn("rounded-full font-mono text-[11px]", STATUS_TONE[cap.supported_status] ?? "")}>
            {cap.supported_status}
          </Badge>
        </div>
        <p className="mt-1 text-sm text-muted-foreground">{cap.canonical_name}</p>
        <p className="mt-2 line-clamp-2 text-sm">{cap.description}</p>
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
        ...
        {open && (
          <dl className="mt-3 space-y-1.5 rounded-md border border-border/70 bg-muted/30 p-3 text-xs">
            {[
              ...
              <div key={k} className="grid gap-0.5 sm:grid-cols-[140px_1fr]">
                <dt className="font-mono text-muted-foreground">{k}</dt>
                <dd className="min-w-0 font-mono text-foreground/90">{v}</dd>
              </div>
            ))}
          </dl>
        )}
...
export function CapabilityExplorer() {
  ...
  useEffect(() => {
    ...
    fetchCapabilities(controller.signal)
      .then(setData)
      .catch(() => setError("Capabilities come from the live registry endpoint (GET /v1/capabilities), which is not reachable from this environment."));
  ...
  const authorities = useMemo(() => {
    if (!data) return [];
    return Object.keys(data.registry_summary).sort();
  }, [data]);

  return (
    <div id="capabilities" className="scroll-mt-20 space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold tracking-tight">Capability registry</h2>
          <p className="mt-0.5 max-w-2xl text-sm text-muted-foreground">
            Read-only discovery over the live registry — the single source of truth.
            UNSUPPORTED and PLANNED are honest boundary records, not errors.
          </p>
        </div>
        <span className="font-mono text-xs text-muted-foreground">GET /v1/capabilities</span>
      </div>

      {error && (
        <p className="rounded-md border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
          {error} Values below are illustrative until the backend is connected.
        </p>
      )}
      ...
      {data && (
        <>
          <div className="flex flex-wrap gap-2">
            {(["ALL", "SUPPORTED", "PARTIAL", "UNSUPPORTED", "PLANNED"] as const).map((s) => (
              <Button
                key={s}
                size="sm"
            ...
            {(["ALL", ...authorities] as const).map((a) => (
              <Button
                key={a}
                size="sm"
                variant={authorityFilter === a ? "default" : "outline"}
