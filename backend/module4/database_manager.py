...
# Import your explicit SQLAlchemy models
from backend.database.models import Company, Financial, MarketPrice, News, Filing, CorporateAction, ExtractedFact
...
class DatabaseManager:
    ...
    def save_financials(self, financials):
        ...
        try:
            ...
            if skipped > 0:
                logger.warning(
                    f"[DB] save_financials skipped {skipped}/{len(records_list)} items "
                    f"— missing company_id or financial_year"
                )
    ...
    # --------------------------------------------------
    # Extracted Facts (Agentic RAG Evidence Store)
    # --------------------------------------------------

    def save_extracted_fact(self, fact: dict) -> Optional[int]:
        """
        Saves an extracted fact to the evidence store.
        Deduplicates by evidence_hash.

        Args:
            fact: Dict with ExtractedFact fields

        Returns:
            The fact ID if saved, or None if duplicate.
        """
        evidence_hash = fact.get("evidence_hash")
        if not evidence_hash:
            logger.warning("[DB] Cannot save extracted fact without evidence_hash")
            return None

        try:
            existing = self.connection.query(ExtractedFact).filter(
                ExtractedFact.evidence_hash == evidence_hash
            ).first()
            if existing:
                logger.info(f"[DB] Extracted fact already exists (hash={evidence_hash[:12]})")
                return existing.id

            new_fact = ExtractedFact(
                company_id=fact.get("company_id"),
                metric_id=fact.get("metric_id") or fact.get("metric_name", ""),
                metric_name=fact.get("metric_name", ""),
                metric_definition=fact.get("metric_definition"),
                metric_value=fact.get("metric_value") or fact.get("value"),
                unit=fact.get("unit"),
                ...
                fiscal_period=fact.get("fiscal_period") or fact.get("reporting_period"),
                ...
                filing_type=fact.get("filing_type"),
                accession_number=fact.get("accession_number"),
                amendment_relationship=fact.get("amendment_relationship"),
                taxonomy=fact.get("taxonomy"),
                evidence_hash=evidence_hash,
                evidence_text_anchor=fact.get("evidence_text_anchor") or fact.get("source_anchor"),
                confidence_score=fact.get("confidence_score") or fact.get("confidence"),
                verification_status=fact.get("verification_status", "PENDING"),
            )
            self.connection.add(new_fact)
            self.connection.flush()
            logger.info(f"[DB] Extracted fact saved (id={new_fact.id}, hash={evidence_hash[:12]})")
            return new_fact.id

        except Exception as e:
            logger.error(f"[DB] Error saving extracted fact: {e}")
            raise

    def query_extracted_facts(
        self,
        ...
        """
        Query extracted facts with optional filters.

        Args:
            company_id: Filter by company
            metric_name: Filter by canonical metric name
        ...
        Returns:
            List of ExtractedFact ORM objects
        """
        try:
            query = self.connection.query(ExtractedFact)

            if company_id is not None:
                query = query.filter(ExtractedFact.company_id == int(company_id))
            if metric_name:
                query = query.filter(ExtractedFact.metric_name == metric_name)
            if metric_id:
                query = query.filter(ExtractedFact.metric_id == metric_id)
            if fiscal_period:
                query = query.filter(ExtractedFact.fiscal_period == fiscal_period)
            if verification_status:
                query = query.filter(ExtractedFact.verification_status == verification_status)

            query = query.order_by(ExtractedFact.created_at.desc()).limit(limit)
            return query.all()

        except Exception as e:
            logger.error(f"[DB] Error querying extracted facts: {e}")
            return []

    def get_extracted_fact_by_hash(self, evidence_hash: str) -> Optional[ExtractedFact]:
        """Retrieve a single extracted fact by its evidence hash."""
        try:
            return self.connection.query(ExtractedFact).filter(
                ExtractedFact.evidence_hash == evidence_hash
            ).first()
        except Exception as e:
            logger.error(f"[DB] Error fetching extracted fact by hash: {e}")
            return None

    def update_extracted_fact_verification(
        self, fact_id: int, status: str
    ) -> bool:
        """Update the verification status of an extracted fact."""
        try:
            fact = self.connection.query(ExtractedFact).filter(
                ExtractedFact.id == fact_id
            ).first()
            if fact:
                fact.verification_status = status
                logger.info(
                    f"[DB] Extracted fact {fact_id} verification: {status}"
                )
                return True
        except Exception as e:
            logger.error(f"[DB] Error updating fact verification: {e}")
            raise
        return False
