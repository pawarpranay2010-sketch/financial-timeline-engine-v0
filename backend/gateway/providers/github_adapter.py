"""GitHub Models adapter — uses GITHUB_TOKEN as credential."""
import time
import os
import requests
...
class GitHubAdapter(ProviderAdapter):
    """GitHub Models — uses GITHUB_TOKEN. Model configurable via GITHUB_MODEL."""

    MODEL = "gpt-4o"

    def __init__(self, api_key: str = ""):
        super().__init__(api_key)
        self.model = os.getenv("GITHUB_MODEL", self.MODEL)
    ...
    def execute(self, prompt: str, system_prompt: str = "",
        ...
        if not self._api_key:
            return NormalizedResponse(content="", provider="github", model=self.model,
                                       error="GITHUB_TOKEN not configured", latency_ms=0)
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                return NormalizedResponse(
                    content=content, provider="github", model=self.model,
    ...
    def capability(self) -> ProviderCapability:
        return ProviderCapability(
            provider="github", model=self.model,
