...
class SambaNovaAdapter(ProviderAdapter):
    """SambaNova Cloud — uses SAMBANOVA_MODEL env var."""

    MODEL = "Meta-Llama-3.3-70B-Instruct"

    def __init__(self, api_key: str = ""):
        super().__init__(api_key)
        self.model = os.getenv("SAMBANOVA_MODEL", self.MODEL)
    ...
    def execute(self, prompt: str, system_prompt: str = "",
        ...
        if not self._api_key:
            return NormalizedResponse(content="", provider="sambanova", model=self.model,
                                       error="SAMBANOVA_API_KEY not configured", latency_ms=0)
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                return NormalizedResponse(
                    content=content, provider="sambanova", model=self.model,
    ...
    def capability(self) -> ProviderCapability:
        return ProviderCapability(
            provider="sambanova", model=self.model,
